...
def metrics_calculation(references_samples,generated_samples,metric,device):

    ### input #####
    ''' references samples/generated samples are essentially a dictionary of the format
      of given here. It has one hatespeech and one or more ground truth/generated counterspeeches presented as a list
    
    dict_samples = {
        '0':{
            'hatespeech':'I hate you',
            'counterspeech':['I love you','I like you']
        }
    
    metric - choose on of the available metrics
    device can be either 'cpu' or 'cuda' preset in the main function.
    '''
...
def heavy_metrics(train_sample,gen_sample,metric,baseline,device):
...
#    gen4        = glob.glob(save_path+'/**'+gen_sample+'*'+'_sz_3_'+'*'+'.json')
    gen5        = glob.glob(save_path+'/*'+train_sample+'*'+gen_sample+'*_huggingface_base.json')
...
#     print(ref_dict)
    ...
    it = 1
    if(metric=='bleurt'):
        bleurt_score=Bleurt(model_path="Elron/bleurt-base-512",cache_path='../Saved_models',max_length=400, batch_size=16,device=device)
    elif(metric=='argument'):
        argument_score=Argument_scoring(model_path='chkla/roberta-argument',cache_path='../Saved_models',max_length=100, batch_size=16,device=device)
    elif(metric=='counterspeech'):
        counterspeech_score=Argument_scoring(model_path='Saved_Models/Discriminator/Counterspeech_bert-base-uncased',cache_path='../../Saved_models',max_length=100, batch_size=16,device=device)
    elif(metric=='upvote_prob'):
        dialog_upvote=Dialog_upvote_scoring(model_path='microsoft/DialogRPT-updown',cache_path='../Saved_models',max_length=100, batch_size=16,device=device)
    elif(metric=='width_prob'):
        dialog_width=Dialog_upvote_scoring(model_path='microsoft/DialogRPT-width',cache_path='../Saved_models',max_length=100, batch_size=16,device=device)
    elif(metric=='depth_prob'):
        dialog_depth=Dialog_upvote_scoring(model_path='microsoft/DialogRPT-depth',cache_path='../Saved_models',max_length=100, batch_size=16,device=device)
    elif(metric=='counter_argument'):
        counter_argument_score=Counter_argument_scoring(model_path='Saved_Models/Discriminator/Argument_bert-base-uncased',cache_path='../Saved_models',max_length=200, batch_size=8,device=device)
    elif(metric=='toxicity'):
        toxicity_score=Toxic_HateXplain_scoring(model_path=None,cache_path='../Saved_models',max_length=100, batch_size=16,device=device)
#         toxicity_score=Toxicity_model(max_length=100, batch_size=8,use_gpu=True)
    ...
    scores={}
    write_in='Metrics_eval_results/Newer_metrics_'+metric+'_on_'+gen_sample+"_"+train_sample+".json"
...
if __name__ == "__main__":
    ...
    heavy_metrics(argument_dataset,dataset,metric,baseline,device)
